...
// o Set parece um array, mas no fim ele é um objeto
...
// no Array comum, para saber e um item existe
...
// na documentação tem exemplos sobre como fazer uma interceção,
// saber o que tem em uma lista e não tem na outra e assim por diante.

// tem nos dois arrays
const users01 = new Set(["fred", "mariazinha", "xuxa da silva"]);

const users02 = new Set(["joaozinho", "fred", "julio"]);

const intersection = new Set([...users01].filter((user) => users02.has(user)));
assert.deepStrictEqual(Array.from(intersection), ["fred"]);
...
const weakSet = new WeakMap([user]);
weakSet.add(user2);
weakSet.delete(user);
weakSet.has(user);
